...
app.controller('userListController',function($rootScope,$scope,$http,$state,$localStorage,$stateParams,$myHttpService,$tableListService,$modal){
    ...
    //清除账户余额
    $scope.refundOrder = function(userid,balance) {
        layer.confirm('您确定要给用户退款吗？', {icon: 3, title:'提示'},function(){
            console.log({userid:userid,refundfee:balance})
            $myHttpService.post("api/user/addUserRefundOrder",{userid:userid,refundfee:balance,refundType: 0},function(data){
                layer.msg(data.msg);
