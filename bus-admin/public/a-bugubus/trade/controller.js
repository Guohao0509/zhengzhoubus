 ...
 */
app.controller('ticketListController',function($rootScope,$scope,$http,$state,$localStorage,$stateParams,$myHttpService,$tableListService,$modal,$filter){
    //全选
    var selected = false;
    ...
    $scope.refundOrder = function(item) {
        layer.confirm('您确定要退款吗？', {icon: 3, title:'提示'},function(){
            var reqParam = {
                userid:item.userId,
                refundType: 1,
                ticketid:item.ticketId,
                refundfee: item.monthPrice,
                startdate: $filter('date')(item.startDate,'yyyy-MM-dd'),
                enddate: $filter('date')(item.endDate,'yyyy-MM-dd')
            }
            console.log(reqParam)
            if(item.isActive == 1){
                $myHttpService.post("api/ticket/addTicketRefundOrder",reqParam,function(data){
                    layer.msg(data.msg);
                    $state.go('app.trade.ticket_list',{},{reload: true});
                });
            }else{
                layer.msg("退款失败");
            }
